...
class CronJob
{
    public function autoSyncronize($sheetName, $sheetId)
    {
        ...
        foreach ($values as $value) {
            ...
            if ($result->num_rows === 0) {
                $queryLead = "INSERT INTO leads (`id`,`date_entered`, `date_modified`,
                    `first_name`, `last_name`, `phone_mobile`, `lead_source`, `birthdate`)
...
$formList =
    "SELECT `g_sheet_id`, `g_sheet_name` FROM `GoogleForm` WHERE deleted = 0";
$result = $db->query($formList, true);

foreach ($result as $formDetails) {
    $sheetName = $formDetails["g_sheet_name"];
    $sheetId = $formDetails["g_sheet_id"];

    $corn = new CronJob();
    $corn->autoSyncronize($sheetName, $sheetId);
}
